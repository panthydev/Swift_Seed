...
public class SeedEvaluator {
    ...
    public void ProcessSeed(long seed) {
        ...
        if (IsStrongholdGood(jobData.strongholdPos)) {
        } else return;
    ...
    private boolean IsStrongholdGood(Position position) {
        if (Position.isWithinRange(position, conditions.maxStrongholdRangeFromOrigin)) {
            return true;
        } else {
            return false;
        }
    ...
    private boolean FindStructure(JobData jobData, long seed) {
        ...
        Cubiomes.getStructurePos(jobData.structure, jobData.version, seed, jobData.strongholdPos.VillageGetRegionX(), jobData.strongholdPos.VillageGetRegionZ(), jobData.pos);
    ...
    private boolean IsStructureGood(Position structurePos, Position StrongholdPos) {
        if (Position.Distance(structurePos, StrongholdPos) > conditions.maxVillageDistFromStronghold) {
            return false;
        } else {
            return true;
        }
